...
class RstLintingProvider {
    ...
    loadConfiguration() {
        ...
        if (!section)
            return;
        var module = [];
        var build = configuration_1.Configuration.loadSetting('linter.executablePath', null, null);
        if (build == null) {
            var python = configuration_1.Configuration.loadSetting('pythonPath', null, null, 'python');
            if (python != null) {
                build = python;
